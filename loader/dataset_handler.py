...
from .object_detection_dataset import ObjectDetectionDataset, ObjectDetectionSplitter, collate_fn_object_detection
...
class DatasetHandler:
    ...
    def process_and_load(self, output_dir: str,
        ...
        if self.is_object_detection_dataset():
            return self._load_object_detection_dataloaders(
                output_path, train_batch_size, val_batch_size, test_batch_size,
                num_workers, pin_memory)
        else:
            # Load processed data for classification datasets
    ...
    def _load_object_detection_dataloaders(self, output_path: Path,
                                         train_batch_size: int,
                                         val_batch_size: int,
                                         test_batch_size: int,
                                         num_workers: int,
                                         pin_memory: bool) -> Tuple[DataLoader, DataLoader, DataLoader]:
        """Load dataloaders for object detection datasets"""
        
        # Create datasets for each split
        train_dataset = ObjectDetectionDataset(
            image_dir=str(output_path / 'train' / 'images'),
            annotation_dir=str(output_path / 'train' / 'annotations'),
            transform=self.transforms['train']
        )
        
        val_dataset = ObjectDetectionDataset(
            image_dir=str(output_path / 'val' / 'images'),
            annotation_dir=str(output_path / 'val' / 'annotations'),
            transform=self.transforms['val']
        )
        
        test_dataset = ObjectDetectionDataset(
            image_dir=str(output_path / 'test' / 'images'),
            annotation_dir=str(output_path / 'test' / 'annotations'),
        ...
        train_loader = DataLoader(
            ...
            shuffle=True, num_workers=num_workers,
            pin_memory=pin_memory, collate_fn=collate_fn_object_detection
        )
    ...
    def _process_structure(self, output_dir: Path,
        ...
        if self.is_object_detection_dataset():
            self._process_object_detection(output_dir, split_ratio)
        elif enforce_split_ratio:
            logging.info(
    ...
    def _process_object_detection(self, output_dir: Path, split_ratio: Tuple[float, float, float] = (0.8, 0.1, 0.1)):
        """Process object detection datasets with separate image and annotation folders"""
        dataset_path = Path(self.config['data_dir']) / self.dataset_name
        
        logging.info(f"Processing {self.dataset_name} object detection dataset")
        ...
        image_path = dataset_path / images_dir
        annotation_path = dataset_path / annotations_dir
        ...
        # Use ObjectDetectionSplitter to split the dataset with progress tracking
        splitter = ObjectDetectionSplitter(str(image_path), str(annotation_path))
        
        # Get total number of images for progress tracking
        total_images = len(splitter.image_paths)
        logging.info(f"Found {total_images} images to process")
        
        # Prepare transforms for each split to resize images to target size
        transforms_dict = {
            'train': self.final_transforms['train'],
            'val': self.final_transforms['val'], 
            'test': self.final_transforms['test']
        }
        
        # Split the dataset with progress tracking and transformations
        splits = splitter.split_dataset_with_progress(str(output_dir), split_ratio, transforms=transforms_dict)
        
        # Create detailed split information for logging
        split_info = {
            'train': {
                'total': len(splits['train']),
                'images': len(splits['train']),
                'annotations': len(splits['train'])  # Assuming 1:1 mapping
            },
            'val': {
                'total': len(splits['val']),
                'images': len(splits['val']),
                'annotations': len(splits['val'])
            },
            'test': {
                'total': len(splits['test']),
                'images': len(splits['test']),
                'annotations': len(splits['test'])
            }
        }
        
        logging.info(f"Object detection dataset split:")
        logging.info(f"  Train: {split_info['train']['total']} images")
        logging.info(f"  Val: {split_info['val']['total']} images")
        logging.info(f"  Test: {split_info['test']['total']} images")
        
        # Log dataset information to file
        self._log_object_detection_dataset_info(output_dir, split_info, image_path, annotation_path)
        
        logging.info(f"Completed processing {self.dataset_name}")

    def _log_object_detection_dataset_info(self, output_dir: Path, split_info: Dict, image_path: Path, annotation_path: Path):
        """Log object detection dataset information to a file"""
        info_file = output_dir / 'dataset_info.txt'
        with open(info_file, 'w') as f:
            f.write(f"Dataset: {self.dataset_name} (Object Detection)\n")
            f.write("=" * 50 + "\n\n")
            
            # Dataset structure information
            f.write("Dataset Structure:\n")
            f.write(f"  - Type: Object Detection\n")
            f.write(f"  - Original Images Directory: {image_path}\n")
            f.write(f"  - Original Annotations Directory: {annotation_path}\n")
            f.write(f"  - Processed Output Directory: {output_dir}\n\n")
            
            # Get original and processed image properties
            sample_image = None
            processed_sample = None
            
            # Find original sample image
            try:
                ...
                logging.error(f"Error finding original sample image: {str(e)}")
            
            # Find processed sample image
            try:
                train_img_dir = output_dir / 'train' / 'images'
                if train_img_dir.exists():
                    for img_file in train_img_dir.glob('*.*'):
                        if img_file.suffix.lower() in ('.jpg', '.jpeg', '.png', '.tif', '.tiff'):
                            processed_sample = img_file
                            break
            except Exception as e:
                logging.error(f"Error finding processed sample image: {str(e)}")
            
            # Write original image properties
            if sample_image:
                try:
                    with Image.open(sample_image) as img:
                        f.write("Original Image Properties:\n")
                        f.write(f"  - Size: {img.size} (W x H)\n")
                        f.write(f"  - Mode: {img.mode}\n")
                        f.write(f"  - Channels: {len(img.getbands())} ({', '.join(img.getbands())})\n")
                        
                        # Calculate file size
                        file_size = sample_image.stat().st_size / 1024  # KB
                        f.write(f"  - Sample File Size: {file_size:.1f} KB\n\n")
                except Exception as e:
                    f.write(f"Error reading original sample image: {str(e)}\n\n")
            else:
                f.write("No original sample image found.\n\n")
            
            # Write processed image properties
            if processed_sample:
                try:
                    with Image.open(processed_sample) as img:
                        f.write("Processed Image Properties:\n")
                        f.write(f"  - Size: {img.size} (W x H)\n")
                        f.write(f"  - Mode: {img.mode}\n")
                        f.write(f"  - Channels: {len(img.getbands())} ({', '.join(img.getbands())})\n")
                        ...
                        # Calculate file size
                        file_size = processed_sample.stat().st_size / 1024  # KB
                        f.write(f"  - Sample File Size: {file_size:.1f} KB\n")
                        
                        # Check if image was resized or transformed
                        if sample_image:
                            with Image.open(sample_image) as orig_img:
                                if orig_img.size != img.size:
                                    f.write(f"  - Resized from: {orig_img.size} to {img.size}\n")
                                if orig_img.mode != img.mode:
                                    f.write(f"  - Mode changed from: {orig_img.mode} to {img.mode}\n")
                        f.write("\n")
                except Exception as e:
                    f.write(f"Error reading processed sample image: {str(e)}\n\n")
            else:
                f.write("No processed sample image found.\n\n")
            
            # Transform information
            f.write("Transform Information:\n")
            f.write("-" * 20 + "\n")
            try:
                # Get transform pipeline from config
                common_config = self.config.get('common_settings', {})
                dataset_config = self.dataset_config
                
                if 'resize' in common_config:
                    f.write(f"  - Target Resize: {common_config['resize']}\n")
                if 'preprocessing' in dataset_config:
                    f.write("  - Preprocessing Applied:\n")
                    for key, value in dataset_config['preprocessing'].items():
                        f.write(f"    * {key}: {value}\n")
                if 'augmentation' in dataset_config:
                    f.write("  - Augmentation Settings:\n")
                    for key, value in dataset_config['augmentation'].items():
                        f.write(f"    * {key}: {value}\n")
                f.write("\n")
            except Exception as e:
                f.write(f"Error reading transform config: {str(e)}\n\n")
            ...
            for split, info in split_info.items():
                f.write(f"\n{split.upper()}:\n")
                f.write(f"  - Images: {info.get('images', 0)}\n")
                f.write(f"  - Annotations: {info.get('annotations', 0)}\n")
                f.write(f"  - Total: {info.get('total', 0)}\n")
                
                # Add directory structure info
                split_dir = output_dir / split
                if split_dir.exists():
                    f.write(f"  - Output Directory: {split_dir}\n")
                    img_dir = split_dir / 'images'
                    ann_dir = split_dir / 'annotations'
                    if img_dir.exists():
                        img_count = len(list(img_dir.glob('*.*')))
                        f.write(f"  - Images Directory: {img_dir} ({img_count} files)\n")
                    if ann_dir.exists():
                        ann_count = len(list(ann_dir.glob('*.*')))
                        f.write(f"  - Annotations Directory: {ann_dir} ({ann_count} files)\n")
            ...
            try:
                for ann_file in annotation_path.glob('*.*'):
                    if ann_file.suffix.lower() in ('.txt', '.xml', '.json'):
                        annotation_formats.add(ann_file.suffix.lower())
                        if sample_annotation is None:
                            sample_annotation = ann_file
                        
                if annotation_formats:
                    f.write(f"  - Detected formats: {', '.join(annotation_formats)}\n")
                    
                    # Try to show sample annotation content
                    if sample_annotation and sample_annotation.suffix.lower() == '.txt':
                        try:
                            with open(sample_annotation, 'r') as ann_f:
                                lines = ann_f.readlines()[:3]  # First 3 lines
                                if lines:
                                    f.write(f"  - Sample annotation format (first 3 lines from {sample_annotation.name}):\n")
                                    for i, line in enumerate(lines, 1):
                                        f.write(f"    Line {i}: {line.strip()}\n")
                        except Exception as e:
                            f.write(f"  - Could not read sample annotation: {str(e)}\n")
                else:
                    f.write("  - No annotation files detected\n")
            except Exception as e:
                f.write(f"  - Error detecting annotation formats: {str(e)}\n")
            
            f.write("\n")
            
            # Dataset configuration summary
            f.write("Dataset Configuration Summary:\n")
            f.write("-" * 20 + "\n")
            f.write(f"  - Dataset Name: {self.dataset_name}\n")
            f.write(f"  - Structure Type: {self.structure_type}\n")
            f.write(f"  - Config Source: {self.dataset_config.get('name', 'Unknown')}\n")
            
            if 'structure' in self.dataset_config:
                structure = self.dataset_config['structure']
                f.write("  - Structure Config:\n")
                for key, value in structure.items():
                    f.write(f"    * {key}: {value}\n")
            
            f.write("\n" + "=" * 50 + "\n")
            
        logging.info(f"Dataset information saved to: {info_file}")
