    ...
    def plot_training_curves(self, save_path=None):
        ...
        if self.loss_components:
            ...
            for loss_type in ['total_loss', 'box_loss', 'obj_loss', 'cls_loss']:
                if loss_type in self.loss_components:
                    epochs = [item['epoch'] for item in self.loss_components[loss_type]]
                    values = [item['value'] for item in self.loss_components[loss_type]]
                    ax.plot(epochs, values, label=loss_type, marker='o', markersize=2)
        ...
        if self.gradient_stats:
            ...
            for stat_type in ['total_norm', 'mean_norm', 'max_norm']:
                if stat_type in self.gradient_stats:
                    epochs = [item['epoch'] for item in self.gradient_stats[stat_type]]
                    values = [item['value'] for item in self.gradient_stats[stat_type]]
                    ax.plot(epochs, values, label=stat_type, marker='o', markersize=2)
        ...
        for i, metric in enumerate(metrics_to_plot):
            if i < 3 and metric in self.metrics_history:  # We have 3 remaining subplots
                ax = axes[1, i]
                epochs = [item['epoch'] for item in self.metrics_history[metric]]
                values = [item['value'] for item in self.metrics_history[metric]]
        ...
        if save_path is None:
            save_path = os.path.join(self.save_dir, f'training_curves_{datetime.now().strftime("%Y%m%d_%H%M%S")}.png')
    ...
    def generate_training_report(self):
        ...
        if 'total_loss' in self.metrics_history:
            ...
            if loss_trend == "stable":
                report['recommendations'].append("Loss is stable - consider adjusting learning rate or trying different optimization strategies")
            elif loss_trend == "degrading":
                report['recommendations'].append("Loss is increasing - consider reducing learning rate or checking for overfitting")
        
        if 'map_50' in self.metrics_history:
            map_values = [item['value'] for item in self.metrics_history['map_50']]
            if all(v == 0 for v in map_values[-5:]):  # Last 5 epochs are 0
                report['recommendations'].append("mAP@0.5 is consistently 0 - check anchor sizes, detection thresholds, or label format")
...
def analyze_dataset_statistics(dataset_loader):
    ...
    if box_sizes:
        ...
        print(f"  Box area - Mean: {np.mean(box_sizes):.4f}, Std: {np.std(box_sizes):.4f}")
        print(f"  Box area - Min: {np.min(box_sizes):.4f}, Max: {np.max(box_sizes):.4f}")
        print(f"  Aspect ratio - Mean: {np.mean(aspect_ratios):.4f}, Std: {np.std(aspect_ratios):.4f}")
        print(f"  Most common classes: {dict(sorted(class_counts.items(), key=lambda x: x[1], reverse=True)[:10])}")
